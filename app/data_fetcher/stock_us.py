...
负责从 Yahoo Finance（yfinance）获取美股实时行情和历史 K线数据。
使用个股直接查询方式，无需全市场下载。

Symbol 格式：纯 ticker（如 AAPL 苹果、TSLA 特斯拉）
"""
...
def get_quote_direct(symbol: str) -> QuoteData:
    """直接查询单个美股实时行情（yfinance）

    Args:
        symbol: 美股代码（如 AAPL 苹果、TSLA 特斯拉）
...
def get_history(
    ...
    Args:
        ...
        adjust: 复权方式（保留以兼容接口，yfinance 自动处理）
